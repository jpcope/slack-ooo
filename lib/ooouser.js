...
class OOOUser {
  ...
  savestate() {
    if (!(this.config.app.savestate && this.config.app.savestate.directory)) {
      return;
    }
    if (this.saving) {
      this.saveAgain = true;
    ...
    var json_str = JSON.stringify({
      ...
      ooo_start: (this.ooo_start ? this.ooo_start.toISOString() : this.ooo_start),
      ooo_end: (this.ooo_end ? this.ooo_end.toISOString() : this.ooo_end)
    });
    var username = this.username;
    var filename = (new Buffer(username, 'utf8')).toString('base64').replace(/\//g, '_');
    var filepath = `${this.config.app.savestate.directory}/${filename}`;
    if (!username) {
      logger.error(`Failed to write ${username}'s info to ${filepath}: invalid username`);
      return;
    }
    if (!filename) {
      logger.error(`Failed to write ${username}'s info to ${filepath}: invalid filename`);
      return;
    }
    if (!json_str) {
      logger.error(`Failed to write ${username}'s info to ${filepath}: invalid contents`);
      return;
    }
    var userObj = this;
    var callback = function(err) {
      var saveAgain = userObj.saveAgain;
      userObj.saveAgain = false;
      userObj.saving = false;
      if (err) {
        logger.error(`Failed to write ${username}'s info to ${filepath}: ${err.toString()}`);
        if (saveAgain) {
          userObj.savestate();
        }
        return;
      }
      logger.info(`Saved user ${username}'s info to ${filepath}`);
      if (saveAgain) {
        userObj.savestate();
      }
    };
    this.saving = true;
    fs.writeFile(filepath, json_str, 'utf8', callback);
  }

  loadstate(filename) {
    var jsonObj = JSON.parse(fs.readFileSync(`${this.config.app.savestate.directory}/${filename}`, 'utf8'));
    if (!jsonObj) {
      logger.warn(`Failed to parse json for user ${this.username} from ${this.config.app.savestate.directory}/${filename}`);
      return false;
    }
    ...
    this.ooo_start = this.load_moment(jsonObj.ooo_start);
    this.ooo_end = this.load_moment(jsonObj.ooo_end);

    if (!(
        ((this.ooo_start ? this.ooo_start.toISOString() : this.ooo_start) === jsonObj.ooo_start) &&
        ((this.ooo_end ? this.ooo_end.toISOString() : this.ooo_end) === jsonObj.ooo_end)
      )) {
      logger.warn(`Failed to verify time-data for user ${this.username} from ${this.config.app.savestate.directory}/${filename}`);
      return false;
    }

    logger.info(`Loaded user ${this.username} from ${this.config.app.savestate.directory}/${filename}`);
    return true;
  }

  load_moment(iso8601_str) {
    var retVal = (iso8601_str ? chrono.parseDate(iso8601_str) : iso8601_str);
    if (retVal) {
      return moment(retVal);
    }
    return retVal;
  }
