...
class OOOUser {
  ...
  loadstate(filename) {
    ...
    if (!jsonObj) {
      logger.warn(`Failed parse json for user ${this.username} from ${this.config.app.savestate.directory}/${filename}`);
      return false;
    }
    ...
    var ooo_start = (jsonObj.ooo_start ? chrono.parseDate(jsonObj.ooo_start) : jsonObj.ooo_start);
    if (ooo_start) {
      ooo_start = moment(ooo_start);
    }
    this.ooo_start = ooo_start;

    var ooo_end = (jsonObj.ooo_end ? chrono.parseDate(jsonObj.ooo_end) : jsonObj.ooo_end);
    if (ooo_end) {
      ooo_end = moment(ooo_end);
    }
    ...
    if (!(
      ...
      logger.warn(`Failed verify time-data for user ${this.username} from ${this.config.app.savestate.directory}/${filename}`);
      return false;
    }
