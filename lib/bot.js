...
class Bot {
  ...
  constructor (config) {
    ...
    if (config.app.savestate && config.app.savestate.directory) {
      var files = fs.readdirSync(config.app.savestate.directory, 'utf8');
      var username, userObj;
      for ( var idx=0, len=files.length; idx < len; idx++ ) {
        username = (new Buffer((files[idx]).replace(/_/g, '/'), 'base64')).toString('utf8');
        userObj = new OOOUser(config, username);
        if (userObj.loadstate(files[idx])) {
          this.ooo_users.set(username, userObj);
        }
      }
    }
  ...
  handleDirectCommand (message) {
    var ooo_user = this.ooo_users.get(message.user);
    // Start a new user
    if (!ooo_user) {
      ooo_user = new OOOUser(this.config, message.user);
      //
      // ooo_user.savestate();
      // not going to save to disk until the user has
      // customized information that needs to persist
      //
      this.ooo_users.set(message.user, ooo_user);
    }

    const response = ooo_user.handleMessage(message.text);
